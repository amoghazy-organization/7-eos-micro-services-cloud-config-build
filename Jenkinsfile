pipeline {
    ...
    environment {
        IMAGE_NAME = "amoghazy/eos-cloud-config-api"
      
    }
    stages {
        stage('Checkout SCM') {
            steps {
                git credentialsId: '', url: 'https://github.com/amoghazy-organization/7-eos-micro-services-cloud-config-build.git', branch: 'main'
            }
        }
                    ...
                    dir('charts') {
                        withCredentials([usernamePassword(credentialsId: 'jfrog-cred', usernameVariable: 'username', passwordVariable: 'password')]) {
                          sh ''' 
                            ...
                            /usr/local/bin/helm package cloud-config-api
                            curl -u ecomadmin:$password -T /home/jenkins/agent/workspace/$JOB_NAME/charts/cloud-config-api-1.0.tgz "https://triallekevd.jfrog.io/artifactory/ecom-helm-local/cloud-config-api-1.0.tgz"
